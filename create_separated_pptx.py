...
OUTPUT_FILE = '/home/marcosgnr/MMT/MMT Specifications/MVP_Module_Summaries.pptx'
...
def add_textbox(slide, left, top, width, height, lines_data):
    ...
    tf = txBox.text_frame
    tf.word_wrap = True
    for i, (text, fsize, bold, color_rgb) in enumerate(lines_data):
...
def make_overview(slide, slide_width):
    ...
    # We start plotting content at Y=1.0 inches because the PPT template has a top header that covers 0 -> 0.8.
    add_rect(slide, 0, Inches(0.85), slide_width, Inches(0.55), NAVY)
    add_textbox(slide, m, Inches(0.9), cw, Inches(0.5), [
        ("MVP Overview — Metering Management Tool", 18, True, WHITE)
    ])
    add_textbox(slide, m, Inches(1.55), cw, Inches(0.5), [
...
def make_module(slide, slide_width, mid, name, obj, tech, feats, todo):
    ...
    add_rect(slide, 0, Inches(0.85), slide_width, Inches(0.55), NAVY)
    add_textbox(slide, m, Inches(0.9), cw, Inches(0.5), [
        (f"{mid} — {name}", 18, True, WHITE)
    ])
    add_textbox(slide, m, Inches(1.55), cw, Inches(0.4), [
...
def main():
    ...
    print("Opened base presentation...")

    # Delete all existing slides
    # (Doing this manually in python-pptx via the element tree)
    xml_slides = prs.slides._sldIdLst
    slides_lst = list(xml_slides)
    for s in slides_lst:
        xml_slides.remove(s)
    
    # Use a basic blank layout (Layout 6 is standard completely blank except BG)
    layout = prs.slide_layouts[6] if len(prs.slide_layouts) > 6 else prs.slide_layouts[-1]
    
    # Find a layout without placeholders if possible to avoid empty text boxes showing up in the template
    for l in prs.slide_layouts:
        if len(l.placeholders) == 0:
            layout = l
            break

    # 1. Add Overview Slide
    slide = prs.slides.add_slide(layout)
    make_overview(slide, prs.slide_width)

    # 2. Add Modules Slides
    order = ["M1", "M2", "M3", "M4", "M5", "M6", "M7", "M8", "M9", "M10", "M11", "M12"]
    for mk in order:
        slide = prs.slides.add_slide(layout)
        m_data = MODULES[mk]
        make_module(slide, prs.slide_width, mk, m_data["n"], m_data["o"], m_data["t"], m_data["f"], m_data["d"])

    prs.save(OUTPUT_FILE)
    print(f"Module summaries generated at {OUTPUT_FILE} successfully with 13 slides!")
