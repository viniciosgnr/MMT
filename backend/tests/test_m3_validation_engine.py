...
class TestTwoSigmaValidation:
    ...
    def test_value_within_band(self):
        check = _check_2sigma("density", 10.1, "kg/m³", self.STABLE_HISTORY)
        assert check.status == "pass"

    def test_value_far_outside_band(self):
        check = _check_2sigma("density", 150.0, "kg/m³", self.STABLE_HISTORY)
        assert check.status == "fail"

    def test_value_negative_outlier(self):
        check = _check_2sigma("density", -50.0, "kg/m³", self.STABLE_HISTORY)
        assert check.status == "fail"

    def test_returns_mean_and_std(self):
        check = _check_2sigma("density", 10.0, "kg/m³", self.STABLE_HISTORY)
    ...
    def test_empty_history_bootstrap(self):
        ...
        check = _check_2sigma("density", 850.0, "kg/m³", [])
        assert check.status == "pass"
    ...
    def test_single_sample_history(self):
        ...
        check = _check_2sigma("density", 100.0, "kg/m³", history)
        assert check.status == "pass"
    ...
    def test_different_parameters(self):
        ...
        check = _check_2sigma("rs", 88.2, "m³/m³", rs_history)
        assert check.status == "pass"
    ...
    def test_highly_volatile_history(self):
        ...
        check = _check_2sigma("density", 60.0, "kg/m³", volatile)
        assert check.status == "pass"
